...
if (load_btn or "analyzed" in st.session_state) and ticker:
    ...
    if "company_data" in st.session_state:
        ...
        with tab2:
            ...
            st.divider()
                        for item in detective.get("items", []):
                border_map = {"green":"2px solid #00a86b","yellow":"2px solid #e6a817","red":"2px solid #e74c3c"}
                border = border_map[item["status"]]
                ico = "green" if item["status"]=="green" else ("yellow" if item["status"]=="yellow" else "red")
                det_preview = item["detail"][:150]
                st.markdown(f'<div style="border-left:{border};padding:0.8rem;margin:0.8rem 0;background:#fafafa;border-radius:4px;"><strong>{ico} {item["name"]}</strong> 评分：{item["score"]:.2f}<br><small>{det_preview}...</small></div>', unsafe_allow_html=True)
