...
if load_btn and ticker:
    demo_tickers = {"600519": "贵州茅台", "300750": "宁德时代", "000002": "万科A", "601318": "中国平安", "000001": "平安银行", "600036": "招商银行"}
...
if (load_btn or "analyzed" in st.session_state) and ticker:
    ...
    with wf.container():
        ...
        cols = st.columns(4)
        for i, (name, icon) in enumerate([("数据采集员","1"),("财务侦探","2"),("公告分析师","3"),("研报撰稿人","4")]):
            with cols[i]:
    ...
    with wf.container():
        ...
        statuses = [
            ("数据采集员","1","数据已获取"),
            ("财务侦探","2",f"{detective_result['red_flags']}项红灯/{detective_result['yellow_flags']}项黄灯"),
            ("公告分析师","3",f"{len(announcements)}条公告"),
            ("研报撰稿人","4","研报已生成"),
        ]
        for i, (name, icon, status) in enumerate(statuses):
            with cols[i]:
                st.markdown(f"**{icon} {name}**  ")
